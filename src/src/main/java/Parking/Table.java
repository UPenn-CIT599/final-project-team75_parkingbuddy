...
public class Table extends Application {
	...
	private void constructTable() {
		TableColumn<ParkingAggregate, String> state = new TableColumn<ParkingAggregate, String>(
				"State");
		...
		TableColumn<ParkingAggregate, String> license = new TableColumn<ParkingAggregate, String>(
				"License");
		license.setMinWidth(250);
		license.setCellValueFactory(
				new PropertyValueFactory<ParkingAggregate, String>("license"));

		TableColumn<ParkingAggregate, Integer> count = new TableColumn<ParkingAggregate, Integer>(
				"Count");
	...
	private TableView<ParkingInstance> constructSubTable(
		...
		List<ParkingInstance> parkingInstances = parkingAggregate
				.getParkingInstances();
		System.out.println(parkingInstances.get(0));

		TableView<ParkingInstance> subTable = new TableView<ParkingInstance>();
		TableColumn<ParkingInstance, LocalDateTime> dateCol = new TableColumn<ParkingInstance, LocalDateTime>(
				"Date Time");
		...
		dateCol.setCellFactory(column -> {
			TableCell<ParkingInstance, LocalDateTime> cell = new TableCell<ParkingInstance, LocalDateTime>() {
		...
		TableColumn<ParkingInstance, BufferedImage> photoCol = new TableColumn<ParkingInstance, BufferedImage>(
				"Photo");
		...
		photoCol.setCellFactory(column -> {
			TableCell<ParkingInstance, BufferedImage> cell = new TableCell<ParkingInstance, BufferedImage>() {
				@Override
